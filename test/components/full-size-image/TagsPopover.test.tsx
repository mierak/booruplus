...
describe('TagsPopover', () => {
	...
	beforeEach(() => {
		fetchTagsSpy = jest.spyOn(thunks.tags, 'fetchTags').mockImplementation(
			(_: string[]): AppThunk<Tag[]> => async (_, __): Promise<Tag[]> => {
				return tags;
			}
		);
	...
	it('Dispatches searchTagOnline()', async () => {
		...
		// then
		expect(store.getActions()).toContainMatchingAction({ type: thunks.tags.searchTagOnline.pending.type, meta: { arg: tags[1] } });
	});
	it('Dispatches searchTagOffline()', async () => {
		...
		// then
		expect(store.getActions()).toContainMatchingAction({ type: thunks.tags.searchTagOffline.pending.type, meta: { arg: tags[1] } });
	});
	it('Dispatches addTag() to onlineSearchForm', async () => {
		...
		fireEvent.click((await screen.findAllByRole('button', { name: 'Add tag to online search' }))[1]);

		// then
		expect(store.getActions()).toContainMatchingAction({ type: actions.onlineSearchForm.addTag.type, payload: tags[1] });
	});
	it('Dispatches addTag() to downloadedSearchForm', async () => {
		// given
		const tagsString = ['tag1', 'tag2', 'tag3'];
		const store = mockStore(
			mState({
		...
		fireEvent.click((await screen.findAllByRole('button', { name: 'Add tag to offline search' }))[1]);

		// then
		expect(store.getActions()).toContainMatchingAction({ type: actions.downloadedSearchForm.addTag.type, payload: tags[1] });
	});
